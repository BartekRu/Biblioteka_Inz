...
def transform_book(row: pd.Series, book_genres: dict) -> dict:
    ...
    doc = {
        ...
        "description": None,
        "average_rating": (
            float(row["average_rating"]) if pd.notna(row.get("average_rating")) else 0.0
        ),
        "ratings_count": int(row["ratings_count"]) if pd.notna(row.get("ratings_count")) else 0,
        "reviews_count": (
            int(row["work_text_reviews_count"])
            if pd.notna(row.get("work_text_reviews_count"))
            else 0
        ),
        "image_url": str(row["image_url"]) if pd.notna(row.get("image_url")) else None,
        "small_image_url": (
            str(row["small_image_url"]) if pd.notna(row.get("small_image_url")) else None
        ),
    ...
    return enrich_book_contract(doc)
...
def main():
    ...
    for doc in books_collection.find(
        {"goodbooks_book_id": {"$exists": True}}, {"goodbooks_book_id": 1}
    ):
